...
def _build_backend(key: str | None) -> dict:
    ...
    if not model:
        return {"analyzer": MockAnalyzer(), "key_found": True,
                "note": f"Groq unavailable ({err}): running the mock baseline."}
    return {"analyzer": GroqAnalyzer(key, model), "key_found": True, "note": None}


def get_backend() -> dict:
    return _build_backend(_secret("GROQ_API_KEY"))
...
with st.sidebar:
    ...
    st.caption(f"GROQ_API_KEY secret detected: {'yes' if backend.get('key_found') else 'no'}")
...
with tab_replay:
    ...
    with left:
        ...
        st.code(redact(json.dumps(fx["context"], indent=2)), language="json", wrap_lines=True)
    with right:
